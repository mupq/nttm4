...
void cbd_add(poly *r, const unsigned char *buf)
{
  ...
  for(i=0;i<KYBER_N/4;i++)
  {
    ...
    r->coeffs[4*i+0] = ((uint32_t) r->coeffs[4*i+0] + a[0] + KYBER_Q - b[0]) % KYBER_Q;
    r->coeffs[4*i+1] = ((uint32_t) r->coeffs[4*i+1] + a[1] + KYBER_Q - b[1]) % KYBER_Q;
    r->coeffs[4*i+2] = ((uint32_t) r->coeffs[4*i+2] + a[2] + KYBER_Q - b[2]) % KYBER_Q;
    r->coeffs[4*i+3] = ((uint32_t) r->coeffs[4*i+3] + a[3] + KYBER_Q - b[3]) % KYBER_Q;
  }
#elif KYBER_ETA == 4
  uint32_t t,d, a[4], b[4];
  int i,j;
  ...
  for(i=0;i<KYBER_N/4;i++)
  {
    ...
    r->coeffs[4*i+0] = (uint32_t)((uint32_t) r->coeffs[4*i+0] + a[0] + KYBER_Q - b[0]) % KYBER_Q;
    r->coeffs[4*i+1] = (uint32_t)((uint32_t) r->coeffs[4*i+1] + a[1] + KYBER_Q - b[1]) % KYBER_Q;
    r->coeffs[4*i+2] = (uint32_t)((uint32_t) r->coeffs[4*i+2] + a[2] + KYBER_Q - b[2]) % KYBER_Q;
    r->coeffs[4*i+3] = (uint32_t)((uint32_t) r->coeffs[4*i+3] + a[3] + KYBER_Q - b[3]) % KYBER_Q;
  }
#else
#error "poly_getnoise in poly.c only supports eta in {3,4,5}"
#endif
