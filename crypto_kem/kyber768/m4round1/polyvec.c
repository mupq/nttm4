...
void polyvec_compress(unsigned char *r, const polyvec *a)
{
  int i;
  for(i=0;i<KYBER_K;i++)
    polyvec_compress_poly(r, &a->vec[i], i);
}
...
void polyvec_decompress(polyvec *r, const unsigned char *a)
{
  int i;
  for(i=0;i<KYBER_K;i++)
    polyvec_decompress_poly(&r->vec[i], a, i);

}

/*************************************************
* Name:        polyvec_compress_poly
*
* Description: Compresses i-th polynomial of a polyvec
*              into a serialized polyvec byte array
*
* Arguments:   - unsigned char *r:  pointer to output serialized byte array
*              - const poly *p:     pointer to i-th polyonimal to be serialized
*              - int i:             index of the poly to be serialized
**************************************************/
...
/*************************************************
* Name:        polyvec_decompress_poly
*
* Description: Decompresses i-th polynomial of a polyvec
*              from a serialized polyvec byte array
*
* Arguments:   - poly *r:           pointer to output polynomial
*              - unsigned char *a:  pointer to input serialized polyvec byte array
*              - int i:             index of the polynomial to be deserialized
**************************************************/
...
void polyvec_pointwise_acc(poly *r, const polyvec *a, const polyvec *b)
{
  ...
  for(j=0;j<KYBER_N;j++)
  {
    ...
    for(i=0;i<KYBER_K;i++)
    {
      t += (uint32_t)b->vec[i].coeffs[j] *a->vec[i].coeffs[j];
    }
    r->coeffs[j] = t%KYBER_Q;
  }
}
