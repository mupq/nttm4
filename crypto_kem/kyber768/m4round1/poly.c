...
extern void pointwise_add(uint16_t *, const uint16_t *, const uint16_t *);
extern void pointwise_sub(uint16_t *, const uint16_t *, const uint16_t *);
...
void poly_pointwise(poly *r, const poly *a, const poly *b)
{
  ...
  for(i=0;i<KYBER_N;i++)
  {
    t = (uint32_t) a->coeffs[i] * b->coeffs[i];
    r->coeffs[i] = t % KYBER_Q;
  }
}
...
void poly_pointwise_acc(poly *r, const poly *a, const poly *b)
{
  ...
  {
    t = (uint32_t) a->coeffs[i] * b->coeffs[i];
    r->coeffs[i] = ((uint32_t) r->coeffs[i] + t) % KYBER_Q;
  }
}
